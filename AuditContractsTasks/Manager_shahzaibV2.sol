...
contract Gateway {
  ...
  modifier onlyMappedToken(address _token, uint32 _standard) {
    require(registry.isTokenMapped(_token, _standard, true), "Gateway: Token is not mapped");_;
    _;
  }
  ...
  function depositERC20(address _token, uint256 _amount) external whanNotPaused returns (uint256) {
    return depositERC20For(msg.sender, _token, _amount);
  }

  function depositERC721(address _token, uint256 _tokenId) external whanNotPaused returns (uint256) {
    return depositERC721For(msg.sender, _token, _tokenId);
  }

  function depositEthFor(address _owner) public payable whanNotPaused returns (uint256) {
  ...
  function depositERC20For(
    ...
    address _token,
    uint256 _amount
  ) public whanNotPaused returns (uint256) {
    require(
      IERC20(_token).transferFrom(msg.sender, address(this), _amount),
      "Gateway: ERC-20 token transfer failed"
    );
  ...
  function withdrawERC20For(
    ...
    uint256 _amount,
    bytes memory _signatures
  ) public whanNotPaused onlyMappedToken(_token, 20) {
    ...
    if (_token == registry.getContract(registry.WETH_TOKEN())) {
      ...
      if (_gatewayBalance < _amount) {
        require(
          ...
          "Gateway: Minting ERC20 token to gateway failed"
        );
      }
  ...
  function withdrawERC721For(
    ...
    uint256 _tokenId,
    bytes memory _signatures
  ) public whanNotPaused onlyMappedToken(_token, 721) {
    ...
    if (!_tryERC721TransferFrom(_token, address(this), _user, _tokenId)) {
      require(
        ...
        "Gateway: Minting ERC721 token to gateway failed"
      );
    }
  ...
  function _createDepositEntry(
    ...
    uint32 _standard,
    uint256 _number
  ) public onlyMappedToken(_token, _standard) returns (uint256 _depositId) {
    (, address _sidechainToken, uint32 _tokenStandard) = registry.getMappedToken(_token, true);
    require(_standard == _tokenStandard);

    DepositEntry storage _entry = DepositEntry(_owner, _token, _sidechainToken, _standard, _number);
